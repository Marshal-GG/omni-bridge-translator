...
class SupportRemoteDataSourceImpl implements ISupportRemoteDataSource {
  ...
  SupportRemoteDataSourceImpl({required this.firestore, required this.storage});
  ...
  Future<List<String>> uploadAttachments(
    ...
    AppLogger.i(
      'Uploading ${attachments.length} attachments for user $userId',
      tag: _tag,
    );
    for (final file in attachments) {
      try {
        ...
        final Reference ref = storage.ref().child(
          '${FirebasePaths.feedbackAttachments}/$userId/$fileName',
        );
  ...
  Future<List<FeedbackTicket>> getTickets(String userId) async {
    ...
    return snapshot.docs
        .map((doc) => FeedbackTicket.fromJson(doc.data(), doc.id))
        .toList();
  }
